...
build-linux: bin-dir
	if [ -z "$(shell git status --porcelain)" ]; then \
  		for TYPE in $(ARCH) $(ARM) $(ARM64) $(I386) ; do \
			sed -i "s|${VERSION}|${VERSION} $(LINUX_OS)/$$TYPE|" $(VERSION_PATH); \
			GOOS=$(LINUX_OS) GOARCH=$$TYPE go build -o $(BIN_DIR)/$(BIN) $(PATH_TO_ENTER_POINT); \
...
build-darwin: bin-dir
	if [ -z "$(shell git status --porcelain)" ]; then \
  		for TYPE in $(ARCH) $(ARM64) ; do \
			sed -i "s|${VERSION}|${VERSION} $(MAC_OS)/$$TYPE|" $(VERSION_PATH); \
			GOOS=$(MAC_OS) GOARCH=$$TYPE go build -o $(BIN_DIR)/$(BIN)  $(PATH_TO_ENTER_POINT); \
...
build-windows: bin-dir
	if [ -z "$(shell git status --porcelain)" ]; then \
		for TYPE in $(ARCH) $(ARM) $(ARM64) $(I386) ; do \
			sed -i "s|${VERSION}|${VERSION} $(WINDOWS_OS)/$$TYPE|" $(VERSION_PATH); \
			GOOS=$(WINDOWS_OS) GOARCH=$$TYPE go build -o $(BIN_DIR)/$(BIN_WINDOWS) $(PATH_TO_ENTER_POINT); \
